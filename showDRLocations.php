...
<table width="250px">
	<thead>
		<th width="30%" class="left">3DR #</th>
		<th width="*%" class="left">Location</th>
	</thead>
	<tbody>
<?php
// Now fetch the table
$qry = $pdo->query('select distinct beadType, baseKey, location from beads where beadType like "%3DR%" and location !="" order by baseKey, location');
while ($row = $qry->fetch()) {
	echo '<tr>';
		echo '<td class="right">' . $row['baseKey'] . '</td>';
