...
$resultid2= mysql_query("select * from beads where store3 > 0  order by beadType , store3 desc, baseKey " ,$Catalog);
...
while ($row= mysql_fetch_assoc($resultid2)) {	// Loop thru result set
	...
	if ($qty>-1) {
		...
		print $row['beadType'] . "-" . $row['beadNo'] ." - " . $row['label'] . "<br/>" .  $row['price'] . " (" . $row['store3'] . ")" ." [" . $qty . "]</td>";	
