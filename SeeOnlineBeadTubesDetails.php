...
<title>Beads By Berrie - Online Inventory Color Chart.</title>
...
// and then loop through the results, outputting a cell for each item
	...
	//$resultid2= mysql_query("select * from beads where online = 'yes'  order by beadType , baseKey , beadNo " ,$Catalog);
	  $resultid2= mysql_query("select * from beads where onLine LIKE '%onl%'  order by beadType ,  beadNo " ,$Catalog);
...
while ($row= mysql_fetch_assoc($resultid2)) {	// Loop thru result set
	...
	if ($qtyTot>-1) {
		...
		print $row['beadType'] . "-" . $row['beadNo']." - " . $row['label'] . "<br/>".  $row['price'] ." [" . $qtyTot . "] <b>[" . $qtyOnl . "]";
